        ...
        Commands::Gateway {
            ...
            async {
                ...
                let paid_work = if let Some(path) = paid_work_config.as_ref() {
                    anyhow::ensure!(
                        output_cache == hellas_rpc::cache::CachePolicy::Off,
                        "--paid-work-config requires --output-cache off; paid execution is not cached or replayed",
                    );
                    anyhow::ensure!(
                ...
                #[cfg(not(feature = "node"))]
                let paid_work = None;
                let provider_trust = if paid_work.is_some() || output_cache == hellas_rpc::cache::CachePolicy::ReplayOnly {
                    None
                } else {
