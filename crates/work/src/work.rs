...
impl From<DeliverError> for Refusal {
    ...
    fn from(error: DeliverError) -> Self {
        ...
        let code = match error {
            ...
            DeliverError::Endpoint(EndpointError::NotAdmitting | EndpointError::CatchingUp) => {
...
impl WorkHandler for WorkService {
    ...
    ) -> Result<impl Into<hellas_rpc::call::WithTrailer<DeliverResultResponse>> + Send, WireStatus>
    {
        ...
        loop {
            // Register before reading state so a completion between the
            // check and await cannot be lost.
            let changed = self.changed.notified();
            tokio::pin!(changed);
            changed.as_mut().enable();
            let response = self.release(&request, &context);
            let waiting = matches!(response.outcome.as_ref(),
                    Some(DeliverOutcome::Refused(refused)) if refused.code == WorkRefusal::NotReady.code() as i32);
            if !waiting || tokio::time::timeout_at(deadline, changed).await.is_err() {
                return Ok(response);
            }
        }
    }
